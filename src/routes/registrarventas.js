...
router.post("/guardar", tcaccion(async (req, res) => {
  ...
  res.send()
  await mandarCorreoRegVentas(r.upsertedCount > 0 ? 0 : 1, usuario, req.body._id)
...
let devuelveFuncionMover = (Reg, msg) => tcaccion(async (req, res) => {
  ...
  res.send()
  await mandarCorreoRegVentas(esRegEliminado ? sobreescribir ? 6 : 5 : sobreescribir ? 4 : 3, usuario, esRegEliminado ? registroAMover.registro._id.valueOf() : de, a)
}, msg)
...
}, "página inválida")).delete(tcaccion(async (req, res) => {
  let fecha = DateTime.fromFormat(req.params.id, "d-M-y")
