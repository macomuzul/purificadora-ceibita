...
}).patch(trycatchruta(async (req, res) => {
...
}).post(trycatchruta(async (req, res) => {
...
router.route('/editar/:nombre').get(trycatchruta(async (req, res) => {
  let datosplantilla = await Plantilla.encontrar({ nombre: req.params.nombre })
  datosplantilla ? res.render('editarplantillas', { datosplantilla }) : res.send("Error, la plantilla que deseas acceder no existe")
}, "La plantilla que deseas editar ya no existe")).patch(trycatchruta(async (req, res) => {
  req.body.ultimaedicion = req.user?.ultimaedicion ?? "usuariodesconocido"
...
router.get('/devuelveplantilla/:nombre', trycatchruta(async (req, res) => {
...
router.get('/devuelvenombreplantillas', trycatchruta(async (req, res) => {
  let plantillas = await Plantilla.find().select("nombre -_id")
  res.send(plantillas?.map(x => x.nombre) || [])
}, "Hubo un error al recuperar la plantilla"))


router.route('/:nombre').delete(trycatchruta(async (req, res) => {
