...
let verificacionIdentidad = require("../security/verificacionIdentidad")
...
}).post(verificacionIdentidad, trycatchruta(async (req, res) => {
  let { usuario } = req.body
  let u = await Usuario.findOne({ usuario })
  if (!u) throw new errorDB("El usuario no existe")
  res.send(u.contraseña)
}, "No se pudo recuperar la contraseña del usuario")).delete(trycatchruta(async (req, res) => {
  let { usuario } = req.body
...
router.route('/usuarios/crear').get(async (req, res) => res.render('crearusuarios')).post(verificacionIdentidad, trycatchruta(async (req, res) => {
...
router.route('/usuarios/editar/:id').get(async (req, res) => res.render('editarusuarios', { nombreUsuario: req.params.id })).post(verificacionIdentidad, trycatchruta(async (req, res) => {
  let { usuario } = req.body
