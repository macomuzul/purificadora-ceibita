...
class graficos extends HTMLElement {
  constructor() {
    ...
    <button class="btn dropdown-toggle form-control" type="button" data-bs-toggle="dropdown" aria-expanded="false">De barra</button>
  ...
  actualizarDatosDropdown() {
    ...
    let labels = [...$(this.graficoProductos).find(".nombreCamionero")].filter((el, i) => checkboxesProductos[i]).map(el => el.textContent);
    // let labels = this.datasets.filter((el, i) => checkboxesDias[i]);
    let datasets = this.datasets.filter((el, i) => checkboxesDias[i]);
    datasets.forEach(el => el.data = el.data.filter((el2, i) => checkboxesProductos[i]));
  ...
  agregarOpciones() {
    let html = `<div class="accordion">
      <div class="accordion-item">
        <h2 class="accordion-header">
          <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#listaCamioneros${contador}">Lista de colores</button>
        </h2>
        <div id="listaCamioneros${contador}" class="accordion-collapse collapse">
          <div class="accordion-body">
          <div class="tituloProductos">Días</div>
            <div class="camioneros graficoDias">`;
    this.datasets.forEach(el => {
      html += `<div class="camionero">
                      <div class="contenedor-color"><input type="color" value="#FFFFFF"></div>
    ...
    html += `</div>
    <div class="contenedorabajo"><button class="actualizarDatos">Actualizar datos</button></div>
          </div>
        </div>
      </div>
    </div>`;
  ...
  crearGrafico(labels, datasets, titulo, type = "bar") {
    this.labels = labels;
    this.datasets = datasets;
    this.titulo = titulo;
    this.$chart = $(this).find("canvas")[0].getContext('2d');
    this.tipo = type;
    this.colocarDatos(this.labels, this.datasets, this.titulo, this.tipo);
    this.agregarOpciones();
  }

  colocarDatos(labels, datasets, titulo, type) {
