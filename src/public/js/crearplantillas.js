qsclickd("#guardar", async function () {
  ...
  let data = JSON.stringify({
    ...
    orden: 1,
    productos: qsarrd("tbody tr").map(fila => ({
...
async function metododropdown(option) {
  let r
  try { r = await fetch(`/plantillas/devuelveplantilla/${option.textContent}`) }
  catch (error) { return mostrarError("No fue posible contactar con el servidor<br>Por favor revisa tu conexión de internet", "Error de conexión") }
  ...
  if (isConfirmed) {
    cambiarHTML(qsd("tbody"), p.map(x => `<tr>
